...
public class CMInterface extends SurfaceView {
    ...
    public CMGameState gameState = new CMGameState(3);
    public String judgeType;

    /////// FIX NEED TO FIND WAY TO GET PLAYER NAME
    CMHumanPlayer humanPlayer = new CMHumanPlayer("randomName");
    ...
    protected void onDraw(Canvas canvas){
        ...
        for(int i=0; i<gameState.getNumFighters().length; i++) {
            drawFighterCard(canvas, 50, 430 + (300*i), gameState.getFighter(i).getName(),
                    gameState.getFighter(i).getPower(), gameState.getFighter(i).getPrizeMoney(), humanPlayer.hasBet[i]);
        }
        ...
        // Finds Y coordinate
        int yCoord = 0;
        if(humanPlayer.f1Clicked) {
            yCoord = 430;
        }
        else if(humanPlayer.f2Clicked) {
            yCoord = 730;
        }
        else if(humanPlayer.f3Clicked) {
            yCoord = 1030;
        }
        else if(humanPlayer.f4Clicked) {
            yCoord = 1330;
        }
        else {
            yCoord = 1630;
        ...
        for(int i=0; i<gameState.getAttachedSpells()[i].size(); i++) {
            if(gameState.getAttachedSpells()[i].get(i).getSpellType() == 'e') {
                ...
                drawSpellCard(canvas, 300 + (250*i), yCoord, gameState.getAttachedSpells()[i].get(i).getName(), gameState.getAttachedSpells()[i].get(i).getMana(),
    ...
    public void setState(CMGameState info) {
    }
}
