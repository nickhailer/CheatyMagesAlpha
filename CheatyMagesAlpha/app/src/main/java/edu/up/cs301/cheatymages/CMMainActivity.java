...
import edu.up.cs301.cheatymages.Players.*;
...
public class CMMainActivity extends GameMainActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CMGameState gameState = new CMGameState(4);
        ...
        if(gameState.getGold()[3] > p4currGold) {
            p2currGold = gameState.getGold()[1];
            p2Gold.setText(p2currGold);
        }
        ...
        if(gameState.getHands()[1].size() != p2currHandSize) {
            ...
            p2Gold.setText(p2currHandSize);
        }
        if(gameState.getHands()[2].size() != p3currHandSize) {
            p3currHandSize = gameState.getHands()[2].size();
            p2Gold.setText(p3currHandSize);
        }
        if(gameState.getHands()[3].size() != p4currHandSize) {
            p4currHandSize = gameState.getHands()[3].size();
            p2Gold.setText(p4currHandSize);
        }

        // Updating opponents Bets
        if(gameState.getBets().get(1) != p2currBets) {
            p2currBets = gameState.getBets().get(1);
            p2Bets.setText(p2currBets);
        }
        if(gameState.getBets().get(2) != p3currBets) {
            p3currBets = gameState.getBets().get(2);
            p2Bets.setText(p3currBets);
        }
        if(gameState.getBets().get(3) != p4currBets) {
            p4currBets = gameState.getBets().get(3);
            p2Bets.setText(p4currBets);
        }
        ...
        // Create a for loop to run number of cards amount of times setting them visable
        for(int i=0; i<gameState.getHands(); i++) {
